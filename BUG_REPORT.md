...
### ISSUE-004: Weight Change Display Is Confusing for Weight Loss

**Reported by:** UX Team
**Severity:** Medium
**Area:** Patient Dashboard - Weight Change Card

**Description:**
When a patient successfully loses weight, the weight change card on the dashboard displays a raw negative number (e.g., "-2.0 kg") which is confusing. Weight loss is a positive health outcome, so the display should clearly present it as an achievement rather than showing a raw negative value.

**Example:**
- Starting weight: 75kg
- Current weight: 73kg
- **Expected display:** "2.0 kg" with a green color and text like "Lost" — showing the absolute value of the change as positive progress
- **Actual display:** "-2.0 kg" — the raw negative number is shown directly, which users interpret as a bad thing

**Hint:** Look at the `calculateWeightChange` utility function in `frontend/src/utils/calculations.ts`. It exists to format weight changes properly, but is it being used where it should be?

**Steps to Reproduce:**
1. Log in as `patient@fitsloth.test`
2. View the dashboard
3. Look at the "Weight Change (7 days)" card
4. If the patient has lost weight, the card shows a raw negative number instead of a clearly formatted positive progress indicator
